...
ggplot(rice.long, aes(x=Date, y=sqrt(value), col=name)) + geom_line(size=0.25) + facet_grid(Set~Model) + theme_classic() + geom_hline(yintercept = sqrt(17.3)^2, col="red", lty=2) + scale_color_manual(values = c("black","grey90", "grey70"), name="Dataset", labels=c("Test", "Train", "Validation")) + scale_linetype(name="Fold") + ylab("RMSE") + theme(axis.text.x = element_text(angle = 90, vjust=0.4))#+ geom_point(size=0.3)
...
ggplot(rice.test, aes(x=Date, y=meanMSE, col = Model)) + 
  geom_line(size=1) + 
  theme_minimal() + 
  scale_color_manual(values = c("orange","blue", "grey70")) + 
